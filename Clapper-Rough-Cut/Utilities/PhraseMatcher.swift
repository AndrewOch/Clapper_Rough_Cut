...
protocol PhraseMatcherProtocol {
    func match(files: [FileSystemElement], projectId: UUID, completion: @escaping (Result<[(FileSystemElement, Phrase)], Error>) -> Void)
    func match(file: FileSystemElement, projectId: UUID, completion: @escaping (Result<[(FileSystemElement, Phrase)], Error>) -> Void)
}
...
final class PhraseMatcher: PhraseMatcherProtocol {
    ...
    func match(file: FileSystemElement, projectId: UUID, completion: @escaping (Result<[(FileSystemElement, Phrase)], Error>) -> Void) {
         match(files: [file], projectId: projectId, completion: completion)
    }
    ...
    func match(files: [FileSystemElement], projectId: UUID, completion: @escaping (Result<[(FileSystemElement, Phrase)], Error>) -> Void) {
        ...
        let body: [String: Any] = [
            ...
            "files": files.map { ["id": $0.id.uuidString, "subtitles": $0.subtitles ?? []] }
        ]
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            do {
                let result = try JSONDecoder().decode([MatchingResponse].self, from: data)
                completion(.success(result.map { ($0.file, $0.phrase) }))
            } catch {
...
//        for file in files {
//            matchFile(file: file, phrases: phrases) { element, phrase in
...
//                let length = textsMatcher.matchingSequenceLength(text1: subtitle.text.lowercased(),
...
//                                    phrases: [Phrase]) -> ([Subtitle], Phrase)? {
